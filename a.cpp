                 ...
                 const mxArray *prhs[])
{
   ...
   typename ImageType::Pointer movingimage
      = ImageType::New();
   typename ImageType::Pointer warped_fix_map
      = ImageType::New();
   typename ImageType::Pointer warped_mov_map
      = ImageType::New();
   ...
   const MatlabPixelType * warpfixminptr =  static_cast<const MatlabPixelType *>(mxGetData(prhs[2]));
   const MatlabPixelType * warpmovminptr =  static_cast<const MatlabPixelType *>(mxGetData(prhs[3]));
   ...
   const unsigned int UseJacFlag = static_cast<unsigned int>( mxGetPr(prhs[2*Dimension+3])[0] );
   ...
   warped_fix_map->SetOrigin( origin );
   warped_fix_map->SetSpacing( spacing );
   warped_fix_map:->SetRegions( region );
   warped_fix_map->Allocate();

   warped_mov_map->SetOrigin( origin );
   warped_mov_map->SetSpacing( spacing );
   warped_mov_map->SetRegions( region );
   warped_mov_map->Allocate();
   ...
   PixelType * warpfixmptr = warped_fix_map->GetBufferPointer();
   PixelType * warpmovmptr = warped_mov_map->GetBufferPointer();
   ...
   while ( fixptr != fixbuff_end )
   {
      *fixptr++ = *fixinptr++;
      *movptr++ = *movinptr++;
      *warpfixmptr++ = *warpfixminptr++;
      *warpmovmptr++ = *warpmovminptr++;
      if (UseJacFlag > 0)
      {
        *jacptr++ = *jacobianptr++; 
      }
   ...
   drfp->SetWarped_fix_Map( warped_fix_map );
   drfp->SetWarped_mov_Map( warped_mov_map );
                 ...
                 const mxArray *prhs[])
{
   ...
   /* Check last two sinput */
   for (int n=2*dim+3; n<2*dim+5; n++)
   {
      if ( !mxIsDouble(prhs[n]) || mxIsComplex(prhs[n]) )
      {
         mexErrMsgTxt("Two last inputs must be double.");
      }
   ...
   if (static_cast<unsigned int>( mxGetPr(prhs[2*dim+3])[0] )  > 0)
   {
       int ii = 2*dim + 2;
